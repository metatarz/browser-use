...
class Controller(Generic[Context]):
	def __init__(
		...
		async def open_tab(params: OpenTabAction, browser: BrowserContext):
			...
			return ActionResult(extracted_content=msg, include_in_memory=True)
		# Human Actions	
		@self.registry.action('Ask human in the loop to request human input for critical decisions, additional information or resume or pause execution', param_model=AskHumanAction)
		async def ask_human(params: AskHumanAction, browser: BrowserContext):
		    await send_message(params.text)
		    ...
		    try:
		        ...
		        if latest_human_response is not None:
		            ...
		            return ActionResult(extracted_content=response_msg, include_in_memory=True)
		        else:
		            timeout_msg = 'AskHumanAction timeout after 30s {latest_human_response}'
		            logger.info(timeout_msg)
		            return ActionResult(extracted_content=timeout_msg, include_in_memory=True)
		
		    except asyncio.TimeoutError:
		        timeout_msg = 'AskHumanAction timeout after 30s'
		        logger.info(timeout_msg)
		        return ActionResult(extracted_content=timeout_msg, include_in_memory=True)
		# Content Actions
		@self.registry.action(
